...
public class ModbusRTU
{
    ...
    public bool Open(string portName, int baudRate, int databits, Parity parity, StopBits stopBits)
    {
        ...
        if (!sp.IsOpen)
        {
            ...
            {
                modbusStatus = "Erro abrindo " + portName + ": " + err.Message;
                return false;
            }
            modbusStatus = portName + " aberta com successo";
            return true;
        }
        else
        {
            modbusStatus = portName + " já está aberta";
            return false;
        }
    }
    public bool Close()
    {
        ...
        if (sp.IsOpen)
        {
            ...
            {
                modbusStatus = "Erro fechando " + sp.PortName + ": " + err.Message;
                return false;
            }
            modbusStatus = sp.PortName + " fechada com successo";
            return true;
        }
        else
        {
            modbusStatus = sp.PortName + " não está aberta";
            return false;
        }
    }
    #endregion
    ...
    public bool SendFc16(byte address, ushort start, ushort registers, short[] values)
    {
        ...
        if (sp.IsOpen)
        {
            ...
            {
                modbusStatus = "Erro no evento de gravação: " + err.Message;
                return false;
            }
            //Evaluate message:
            if (CheckResponse(response))
            {
                modbusStatus = "Gravado com successo";
                return true;
            }
            else
            {
        ...
        {
            modbusStatus = "Porta Serial fechada";
            return false;
        }
    }
    #endregion
    ...
    public bool SendFc3(byte address, ushort start, ushort registers, ref short[] values)
    {
        ...
        if (sp.IsOpen)
        {
            ...
            {
                modbusStatus = "Erro no evento de leitura: " + err.Message;
                return false;
            }
            //Evaluate message:
            if (CheckResponse(response))
            {
                ...
                modbusStatus = "Leitura com successo";
                return true;
            }
            else
            {
